...
#Notice that the valid values for period are those defined for the relativedelta relative information, 
#namely: 'years', 'months', 'weeks', 'days', 'hours', 'minutes', 'seconds', 'microseconds'.
...
folder_path = '/content/drive/MyDrive/Colab Notebooks/ISGS/uc06/data 30'
...
for filename in filenames:
    ...
    credit_uc6 = pd.read_csv(os.path.join(folder_path, filename), sep=";")
    print(os.path.join(folder_path, filename))
    
    # Do any necessary processing on the DataFrame
    def random_dates(start, end, n=30):
        start_date = start.value//10**9
        end_date = end.value//10**9
    
        return pd.to_datetime(np.random.randint(start_date, end_date,n),unit='s')

    start = pd.to_datetime(datetime.now()+ relativedelta(days=-1))
    end = pd.to_datetime(datetime.now())
    data_tgl1 = random_dates(start, end)
    tgl_trx1 = pd.DataFrame(data_tgl1)
    tgl_trx = pd.concat([tgl_trx1], axis=1)
    tgl_trx.columns =['Credit_Card_Trx_Datetime']
    credit_uc6['Credit_Card_Trx_Datetime'] = tgl_trx['Credit_Card_Trx_Datetime']
